...
// Limite digit quantità (esclude separatore), max 10
...
qtyEl.addEventListener('input', (e) => {
  ...
  if (san !== val) {
    ...
    e.target.value = san;
    // riposiziona il cursore il più possibile
    try { e.target.setSelectionRange(pos - (val.length - san.length), pos - (val.length - san.length)); } catch {}
  }
});
...
formEl.addEventListener('submit', async (e) => {
  ...
  const qty = sanitizeQtyInput(qtyEl.value || '').trim(); // sanitizzata
  const unit = (unitEl.value || '').trim();
  if (!text) return;

  const item = { id: uuid(), text, done: false };
  if (qty) item.qty = qty.replace(',', '.'); // normalizza la virgola
